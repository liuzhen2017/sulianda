...
public class BusiOrderController extends BaseController {
    ...
    @Log(title = "草稿订单重新提交", businessType = BusinessType.UPDATE)
    @PostMapping("/resubmitDraft/{id}")
    public AjaxResult resubmitDraft(@PathVariable Long id) {
        return toAjax(busiOrderService.resubmitDraftOrder(id));
    }
